...
const ProofOfResidenceThirdFileComponent = ({ data }) => (
    <Document>
        <Page size="A4" style={styles.page}>
            <View>
                <Text style={styles.title}>DECLARAÇÃO DE RESIDÊNCIA</Text>
                <Text>
                    Eu, {data.nameThird}, portador(a) do RG nº {data.identityThird} Órgão Exped. {data.identityEntityThird}/{data.stateIdentityThird} e CPF nº {getCpfFormated(data.cpfThird)}, DECLARO 
                    para fins de comprovação de residência, sob penas da Lei (art. 2° da Lei 7.115/83), que {data.name}, portador(a) do RG nº {data.identity} Órgão Exped. {data.identityEntity}/{data.stateIdentity} e
                    CPF nº {getCpfFormated(data.cpf)}, é residente e domiciliado(a) em meu endereço na {data.streetAddress}, nº {data.houseNumber}, {data.complement ? `${data.complement}, ` : ''} bairro {data.neighborhood},
...
export default ProofOfResidenceThirdFileComponent;